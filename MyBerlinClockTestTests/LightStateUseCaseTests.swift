...
final class LightStateUseCaseTests: XCTestCase {
    ...
    func test_oneSecondLight_expectedResult() {
        ...
        XCTAssertFalse(useCase.secondState(with: createDateWith(second: 1)))
        XCTAssertFalse(useCase.secondState(with: createDateWith(second: 3)))
        XCTAssertFalse(useCase.secondState(with: createDateWith(second: 5)))
        XCTAssertFalse(useCase.secondState(with: createDateWith(second: 7)))
        XCTAssertFalse(useCase.secondState(with: createDateWith(second: 9)))

        // even
        
        XCTAssertTrue(useCase.secondState(with: createDateWith(second: 0)))
        XCTAssertTrue(useCase.secondState(with: createDateWith(second: 2)))
        XCTAssertTrue(useCase.secondState(with: createDateWith(second: 4)))
        XCTAssertTrue(useCase.secondState(with: createDateWith(second: 6)))
        XCTAssertTrue(useCase.secondState(with: createDateWith(second: 8)))
        XCTAssertTrue(useCase.secondState(with: createDateWith(second: 10)))
    }
    ...
    func checkFiveHoursSatisfy(with hours: ClosedRange<Int>, expectedResult: [Bool]) {
        let results = hours.map { useCase.fiveHoursRowStates(with: createDateWith(hour: $0)) }
    ...
    func checkOneHourSatisfy(with hours: [Int], expectedResult: [Bool]) {
        let results = hours.map { useCase.oneHourRowStates(with: createDateWith(hour: $0)) }
        XCTAssert(results.allSatisfy { $0 == expectedResult })
    }
    ...
    func checkFiveMinutesSatisfy(with minutes: ClosedRange<Int>, expectedResult: [Bool]) {
        let results = minutes.map { useCase.fiveMinutesRowStates(with: createDateWith(minute: $0)) }
    ...
    func checkOneMinuteSatisfy(with minutes: [Int], expectedResult: [Bool]) {
        let results = minutes.map { useCase.oneMinuteRowStates(with: createDateWith(minute: $0)) }
...
extension LightStateUseCaseTests {
    func createDateWith(hour: Int = 0, minute: Int = 0, second: Int = 0) -> Date {
